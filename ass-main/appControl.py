...
def systemInfo():
	...
	info = ["Tổng dung lượng đĩa: " + str(round(int(my_system_1.Size)/(1024**3),2)) + " GB",
			"Dung lượng đĩa trống: " + str(round(int(my_system_1.Freespace)/(1024**3),2)) + " GB",
			"Manufacturer: " + my_system_2.Manufacturer,
			"Model: " + my_system_2. Model,
			"Người dùng: " + my_system_2.PrimaryOwnerName,
...
def batteryInfo():
	...
	if battery.power_plugged:
		return "Pin đang sạc đã được " + pr + "% ."
	return "Pin còn " + pr + "%."

def OSHandler(query):
	if isContain(query, ['thiết bị', 'thông tin']):
		return ['Thông tin thiết bị của ngài...', '\n'.join(systemInfo())]
	elif isContain(query, ['cpu', 'pin']):
		return batteryInfo()
